use iced::{Color, Background, Border, Shadow, Vector, Theme};
use iced::widget::{button, container, text_input, text};
...
pub fn primary_button_style() -> Box<dyn Fn(&Theme, button::Status) -> button::Style> {
    Box::new(|_theme: &Theme, _status: button::Status| {
        button::Style {
...
pub fn input_style() -> Box<dyn Fn(&Theme, text_input::Status) -> text_input::Style> {
    Box::new(|_theme: &Theme, _status: text_input::Status| {
        text_input::Style {
