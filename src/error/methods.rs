...
impl AppError {
    ...
    pub fn storage(operation: &str, message: &str) -> Self {
        Self::Storage { operation: operation.to_string(), path: None, message: message.to_string() }
    }
    pub fn storage_with_path(operation: &str, path: &str, message: &str) -> Self {
        Self::Storage { operation: operation.to_string(), path: Some(path.to_string()), message: message.to_string() }
    }
    pub fn crypto(operation: &str, message: &str) -> Self {
        Self::Crypto { operation: operation.to_string(), message: message.to_string() }
    }
    ...
    pub fn suggestion(&self) -> String {
        match self {
            ...
            Self::Api { endpoint, .. } => {
                format!("{}: {endpoint}", t("Lỗi kết nối API", "API connection failed"))
            }
            Self::Storage { operation, path, .. } => {
            ...
            Self::Crypto { operation, .. } => {
                match operation.as_str() {
                    ...
                    _ => t("Kiểm tra lại passphrase hoặc backup", "Check your passphrase or backup").to_string(),
                }
            }
            Self::Unknown { .. } => {
                t("Thử lại hoặc khởi động lại ứng dụng", "Try again or restart the application").to_string()
            }
        }
    }
