use crate::i18n::t;
use crate::ui::theme::{text_scaled, secondary_button_style, selected_button_style, sidebar_style, text_color, text_primary_color, text_secondary_color, Colors};
use iced::{widget::{button, column, container, row, Space, text}, Alignment, Element, Length, Padding};
use iced_fonts::{BOOTSTRAP_FONT, Bootstrap};

use super::structure::{NavItem, Sidebar, SidebarMessage, SidebarEvent};
...
impl Sidebar {
    ...
    pub fn view(&self, wallet_count: usize, compact: bool) -> Element<'_, SidebarMessage> {
        ...
        let nav_items: Element<_> = column(
            NavItem::all().into_iter().map(|item| {
                ...
                let item_row = if item == NavItem::Wallets && wallet_count > 0 {
                    let badge = container(text_scaled(format!("{}", wallet_count), 11).style(text_primary_color())).padding(Padding::from([2, 6]));
                    row![icon, Space::with_width(8), title, Space::with_width(8), badge].align_y(Alignment::Center)
                } else {
                    row![icon, Space::with_width(12), title].align_y(Alignment::Center)
                };

                let style = if is_active { selected_button_style() } else { secondary_button_style() };
                button(item_row).on_press(SidebarMessage::Navigate(item)).padding(item_padding).width(Length::Fill).style(style).into()
            }).collect::<Vec<_>>(),
        )
        ...
        container(content).width(220).height(Length::Fill).style(sidebar_style()).into()
    }
}
